...
bool Texture::Load() {
  ...
  if (format == -1) {
    std::cout << "No se encontró la imagen: " << m_fileName << " - Abortando."
              << std::endl;
    exit(-1);
  }
  ...
  if (format == FIF_UNKNOWN) {
    ...
    if (!FreeImage_FIFSupportsReading(format)) {
      std::cout << "El formato de la imagen no puede ser leeído!" << std::endl;
      exit(-1);
    }
  }
  ...
  // Se indica el tipo de interpolación para ajustar la imagen que se cargo a la GPU
