...
/**
 * Método que actualiza el buffer de vertices, cuando cambia se actualizán los vertices de la escena.
 * @param vertices Vertices del modelo.
 * @param length Tamaño de Vertices.
 */
void Model::UpdateBuffer(Vertex3 vertices[], int length) {
  Vertices = vertices;

  glBindBuffer(GL_ARRAY_BUFFER, VertexBufferId);
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vertices[0]) * length, NULL,
  GL_STREAM_DRAW);
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(Vertices[0]) * length, Vertices);
