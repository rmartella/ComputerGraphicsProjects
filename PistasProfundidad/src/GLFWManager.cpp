...
int main() {
  ...
  // Se crean los objetos aplicación y el objeto que tiene las funcionalidades de pistas de profundidad.
  rayCasting = new RayCasting::RayCasting();
  ...
  rayCasting->SetCamera(pCamera);
  application->SetRayCasting(rayCasting);
...
int GLFWManager::Initialize(int width, int height, std::string strTitle,
  ...
  // Se colocan las dimenciones de la pantalla y que el objeto rayCasting sepa cuantos rayos
  // Se van a genera.
  rayCasting->xBottom = 0;
  rayCasting->xTop = width;
  rayCasting->yBottom = 0;
  rayCasting->yTop = height;
...
bool GLFWManager::ProcessInput(bool continueApplication = true) {
  ...
  if (glfwGetKey(Window, GLFW_KEY_2))
    rayCasting->setContrast(true);
  if (glfwGetKey(Window, GLFW_KEY_3)) {
    if (rayCasting->getOffset() - 1.0 > 0)
      rayCasting->setOffset(rayCasting->getOffset() - 1.0);
  }
  if (glfwGetKey(Window, GLFW_KEY_4))
    rayCasting->setOffset(rayCasting->getOffset() + 1.0);
  if (glfwGetKey(Window, GLFW_KEY_5)) {
    if (rayCasting->getScale() - 0.1 > 0)
      rayCasting->setScale(rayCasting->getScale() - 0.1);
  }
...
void GLFWManager::ReshapeGL(GLFWwindow* Window, int widthRes, int heightRes) {
  ...
  pCamera->SetPerspective(
      glm::radians(60.0f),
      application->ScreenWidth / (double) application->ScreenHeight,
      GLApplication::nearPlane, GLApplication::farPlane);

  rayCasting->xBottom = 0;
  rayCasting->xTop = widthRes;
  rayCasting->yBottom = 0;
  rayCasting->yTop = heightRes;
